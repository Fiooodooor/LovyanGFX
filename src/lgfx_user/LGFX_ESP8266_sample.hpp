...
// Configuration example for using LovyanGFX with custom settings on ESP8266
...
/*
 ...
 for example, if configuring an ILI9341 with SPI connection on ESP8266,
...
// SPI bus settings
      ...
      cfg.spi_3wire  = false;       // Set to true if reception is done via the MOSI pin
      ...
      cfg.pin_dc   =  4;            // Set SPI D/C pin number  (-1 = disable)
    ...
    { // Configure display panel control settings.
      ...
      cfg.pin_rst          =    16;  // Pin number connected to RST  (-1 = disable)
